	...
	public class ArenaRun
	{
		...
		public string PackString
		{
			get
			{
				...
				return packs.Any() ? packs.Select(x => EnumDescriptionConverter.GetDescription(x)).Aggregate((c, n) => c + ", " + n) : "无";
			}
		}
		...
		public string CardString
		{
			get
			{
				...
				return cards.Any()
						   ? cards.Select(x => (Database.GetCardFromId(x.CardId).LocalizedName) + (x.Golden ? " (金卡)" : ""))
								  .Aggregate((c, n) => c + ", " + n) : "无";
			}
		}
		...
		public string DurationString => Duration + " 分";
